...
# Stores only the columns for subject ID, session ID, and repetition number
x.identifiers <- subset( X, subset = TRUE, select = c( subject, sessionIndex, rep ) );
...
# Rejoin the identifying information to the timing data
X <- data.frame( x.identifiers, X );
...
all.num.training.examples <- c( 10, 15, 25, 50, 100, 200 );
for( num.training.examples in all.num.training.examples ) {
for( detectorName in names( detectorSet ) ) {

  #######################
  cat("Evaluating the",detectorName,"detector with",num.training.examples,"training examples.\n");
  ...
  for( i in 1:n ) {
    ...
    eer <- evaluateSubject( X, subjects[i],
                           ...
                           numTrainingExamples = num.training.examples );
